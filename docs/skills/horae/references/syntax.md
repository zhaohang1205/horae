...
```sh
horae capture "给妈妈订蛋糕 @calls ~周六 10:00 !b"
...
```
FREQ=DAILY|WEEKLY|MONTHLY;INTERVAL=2;BYDAY=MO,WE;BYMONTHDAY=1,-1;COUNT=10|UNTIL=YYYY-MM-DD
```

- `BYMONTHDAY=-1` = 月末最后一天；`COUNT` 与 `UNTIL` 二选一。
- 引擎仅支持 **DAILY/WEEKLY/MONTHLY**（366 天展开视野，进程内实现，无外部 crate）。
...
- `*y` / `FREQ=YEARLY` **不受展开引擎支持**。
- TUI 输入层与 CLI（`capture` / `schedule --rrule`）**一致拒绝**，写库前报错并提示
  支持的三种频率——不会出现"存进去了却不重排"的静默退化。
- 结论：年循环习惯（如年度体检）请用"月循环 + 备注"或手动重排替代。

## 组合速查

| 需求 | 一句话 |
| --- | --- |
| 今天下班前要交 | `写周报 @work ~今天 18:00 !a` |
| 每两周周一站会 | `站会 @work *2w[1] ~周一 09:30` |
| 每月底对账 | `对账 @computer *m[-1] ~21:00` |
| 高优但没想好时间 | `联系牙医 @calls !a`（留在收件箱待厘清） |
