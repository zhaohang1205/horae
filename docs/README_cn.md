...
循环 RRULE（一句话里 `*` 简写）：`FREQ=DAILY|WEEKLY|MONTHLY|YEARLY`、`INTERVAL=2`、`BYDAY=SA,SU`、`BYMONTHDAY=1,-1`（-1=月末最后一天）、`BYMONTH=1,7`（按月份数字或名称，如 `jan,jul`）、`COUNT=10` / `UNTIL=YYYY-MM-DD`。快速简写：`*d`/`*w`/`*m`/`*y`（每天/周/月/年）、`*weekday`/`*weekend`（工作日/周末）、`*2w[1,3]`（每两周周一、周三，1-7=周一至周日，0=周日）、`*m[1,-1]`（每月 1 号和最后一天，负数=月末倒数）、`*m[1,15]`（每月 1 号、15 号）、`*y[jan,jul]`（每年 1 月与 7 月）、`*2y[6]`（每两年 6 月），输入 `*2`/`*3` 动态推导 `*2d`/`*2w` 等。优先级支持 `!high`/`!h`/`!1`/`!高`、`!medium`/`!m`/`!2`/`!中`、`!low`/`!l`/`!3`/`!低`（大小写均可）。补全支持 `Alt+1`~`Alt+9` 快捷直选与缺失槽位引导。
