...
impl<'a> App<'a> {
    ...
    pub(super) fn confirm_search(&mut self, input: &str) -> Result<()> {
        ...
        if self.search_query.is_empty() {
            self.status_message = tr!(self.lang, "已清除搜索", "Search cleared").into();
            ...
            self.status_message = tr!(self.lang, "搜索: {}", "Search: {}", self.search_query);
        }
        self.reload()
    }
    ...
    pub(super) fn confirm_filter_tag(&mut self, input: &str) -> Result<()> {
        ...
        if t.is_empty() {
            ...
            self.status_message = tr!(self.lang, "已清除标签过滤", "Tag filter cleared").into();
        } else {
            self.tag_filter = Some(t.clone());
            self.status_message = tr!(self.lang, "过滤标签: @{}", "Filter tag: @{}", t);
        }
        self.reload()
    }
    ...
    pub(super) fn validate_capture_input(&self, input: &str) -> Result<(), String> {
        let quick_add = horae_core::parser::parse_quick_add(input);
        if let Some(rr) = &quick_add.rrule {
            if !horae_core::parser::rrule_valid(rr) {
                return Err(tr!(self.lang, "循环无效: {}", "bad rrule: {}", rr).to_string());
            }
        }
        if let Some(ts) = &quick_add.time_str {
            let parsed = time::parse_time(ts)
                .map_err(|e| tr!(self.lang, "时间无效: {}", "bad time: {}", e).to_string())?;
            if quick_add.rrule.is_some() && parsed < horae_core::time::now_ms() {
                return Err(tr!(
                    self.lang,
                    "循环任务的时间需在未来 ({} 已过去)",
    ...
    pub(super) fn confirm_capture(&mut self, input: &str) -> Result<()> {
        ...
        let quick_add = horae_core::parser::parse_quick_add(raw_input);
        if let Some(id) = self.organizing_id.take() {
            ...
            if let Some(rr) = &quick_add.rrule {
                if !horae_core::parser::rrule_valid(rr) {
                    self.status_message = tr!(self.lang, "循环无效: {}", "bad rrule: {}", rr);
                    self.reload()?;
                    return Ok(());
                }
            }
            ...
            for tg in &old_tags {
                if !new_set.contains(&tg.name) {
                    ok &= self.note(horae_core::repo::tags::remove_tag_from_task(
                        self.conn, &id, &tg.name,
                    ));
                }
            }
            for name in &tag_names {
                ok &= self.note(horae_core::repo::tags::add_tag_to_task(
                    self.conn, &id, name,
                ));
            }
            // 优先级为独立字段，不再混入标签。
        ...
        if let Some(row) = self.items.get(self.selected).cloned() {
            match time::parse_time(start_s) {
                Ok(start_ms) => {
                    ...
                    self.status_message =
                        tr!(self.lang, "{} -> 等待中", "{} -> waiting", short_id(&t.id));
                    self.reload()?;
                }
                Err(e) => self.status_message = tr!(self.lang, "时间无效: {}", "bad time: {}", e),
            }
        }
        Ok(())
    }
        ...
        if !input.is_empty() {
            if let Some(row) = self.items.get(self.selected).cloned() {
                if tasks::add_checklist_item(self.conn, &row.id, input).is_ok() {
                    self.status_message = tr!(self.lang, "检查单 +1", "Checklist +1").to_string();
                }
                self.load_detail();
            }
        }
    ...
    pub(super) fn confirm_creating_tag(&mut self, input: &str) -> Result<()> {
        ...
        if !name.is_empty() && tags::find_or_create_tag(self.conn, name).is_ok() {
            self.status_message = tr!(self.lang, "已创建标签: {}", "created tag: {}", name);
            self.refresh()?;
        }
        self.set_mode(Mode::Normal);
        self.input.clear();
    ...
    pub(super) fn confirm_pomo_config(&mut self, input: &str) -> Result<()> {
        ...
        let i = if parts.len() == 4 {
            ...
            Ok(horae_core::model::pomodoro::PomoConfig::default().long_break_interval)
        };

        if let (Ok(w), Ok(s), Ok(l), Ok(i)) = (w, s, l, i) {
            if w > 0 && s > 0 && l > 0 && i > 0 {
                let mut pomo = horae_core::repo::pomodoro::get_state().unwrap_or_default();
                pomo.config.work_mins = w;
                pomo.config.short_break_mins = s;
                pomo.config.long_break_mins = l;
                pomo.config.long_break_interval = i;
                if horae_core::repo::pomodoro::save_state(&pomo).is_ok() {
                    self.status_message = tr!(
                        self.lang,
