# CONTEXT.md — horae 领域词汇表

GTD 终端任务管理器（`horae`）。核心设计理念是**时间数据化**：每个任务状态变更都盖 UTC-ms 时间戳并追加进只增的 `task_events` 时间线。所有变更经 **变更+审计 seam（`repo::mutate`）** 在单一事务里完成——它打开事务、计算唯一的 `now`、跑变更闭包、提交/回滚；每次变更及其 `log_event` 共享同一个 `now`（时间数据化的一致性由 seam 保证）。
