...
- `repo/` — rusqlite data access; `tasks.rs` holds most domain logic (`create_capture`, `transition`, `schedule`, `resolve_id`, `list`). `mod.rs::log_event` writes the audit timeline.
