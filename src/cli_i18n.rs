...
fn zh_arg_help(path: &str, arg: &str) -> Option<&'static str> {
    ...
    Some(match (path, arg) {
        ...
        ("horae", "lang") => "帮助文本的输出语言：`en`（默认）或 `zh`（中文）。也可通过环境变量 `HORAE_LANG` 设置。",
        ...
        ("schedule", "end") => "计划结束时间",
        ("schedule", "rrule") => "重复规则（FREQ=DAILY|WEEKLY|MONTHLY;INTERVAL=..;BYDAY=..;COUNT=..|UNTIL=..）",
...
mod tests {
    ...
    fn localize_overrides_top_level_and_subcommand_to_chinese() {
        ...
        assert_eq!(cmd.get_about().map(|s| s.to_string()), Some("GTD 终端任务管理器".to_string()));
        let capture = cmd
            .get_subcommands()
